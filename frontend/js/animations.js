	...
	if ($("#mobile-indicator").css("display") == "none" && !$("body").hasClass("animated")) {
		...
		$(".section-top .anim").each(function (index, element) {
			...
			let elTween = TweenMax.to($(element), 1 * animMultiplier, {
				...
				ease: Sine.easeOut,
				// overwrite: "none",
				delay: $(element).data("anim-index") * .5 * animMultiplier + .5 * animMultiplier + 1
			});
		...
		$("main .section-header > *").each(function (index, element) {
			...
			let elScene = new ScrollMagic.Scene({
				triggerElement: element.closest(".section-header"),
				offset: -200,
				reverse: false
			})
				.setTween(elTween)
		...
		$(".about-pic-pic").each(function (index, element) {

			let elTween = TweenMax.to($(element), 2 * animMultiplier, {
				y: 0,
				x: 0,
				scale: 1,
				opacity: 1,
		...
		$(".fabric-teasers-tmb").each(function (index, element) {

			let elTween = TweenMax.to($(element), 1.4 * animMultiplier, {
				y: 0,
				x: 0,
				scale: 1,
				opacity: 1,
		...
		$("main .des-pros-tmb").each(function (index, element) {
			...
			let elScene = new ScrollMagic.Scene({
				triggerElement: element.closest(".des-pros-list"),
				offset: 0,
				reverse: false
			})
				.setTween(elTween)
...
$(window).on("resize scroll load", function () {
	...
	$(".section-pic img").each(function () {
		...
		$(this).prlx($(this).closest(".section"), -100, 100, false)
